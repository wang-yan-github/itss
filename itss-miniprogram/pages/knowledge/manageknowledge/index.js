...
Page({
    ...
    showaction: function (options) {
        ...
        var point = options.currentTarget.dataset.point;
        console.log(point);
        let arr = [];
        if(permission.book_manage_point){
            if(status == 2 && point == null  ){
              arr.push({name: '打分'});
            }
        }
        arr.push({name: '删除'});
        if(permission.book_manage_use){
            if(status == 2 ){
              arr.push({name: '使用'});
            }
        }
        if(permission.book_manage_unuse){
            if(status == 2 ){
              arr.push({name: '禁用'});
            }
        }
    ...
    onSelect(event) {
        ...
        var id = that.data.id
        //console.log(event.detail);
        if (event.detail.name == "打分") {
            wx.navigateTo({
                url: '../examineknowledge/point/index?id=' + id+'&type=1',
            })
        } else if (event.detail.name == "删除") {
            wx.request({
                url: app.globalData.address + 'knowledge/deleteKnowledge.do',
                method: 'POST',
                header: {
        ...
        } else if (event.detail.name == "使用") {

            wx.request({
                url: app.globalData.address + 'knowledge/useKnowledge.do',
                method: 'POST',
                header: {
                  "Authorization":wx.getStorageSync('tokenValue'),
                  "Content-Type": "application/x-www-form-urlencoded"
        ...
        } else if (event.detail.name == "禁用") {
            wx.request({
                url: app.globalData.address + 'knowledge/unUseKnowledge.do',
                method: 'POST',
                header: {
                  "Authorization":wx.getStorageSync('tokenValue'),
                  "Content-Type": "application/x-www-form-urlencoded"
    ...
    onShow: function () {
        ...
        wx.request({
            url: app.globalData.address + 'knowledge/manageKnowledge.do',
            method: 'POST',
            header: {
                "Authorization": wx.getStorageSync('tokenValue'),
                "Content-Type": "application/x-www-form-urlencoded"
