...
Page({
        ...
        if (obj.change_type == "1") {
          ...
          obj.changeNodes.forEach((item, i) => {
            if (item.time_nodes == "1") {
              ...
              that.data.stepData.push({text: "审批", desc: item.update_time==null? "" :item.update_time});
  ...
  saveClick: function (event) {
    ...
    if (!that.data.approve_remark) {
      ...
      that.data.form.changeNode.approve_remark = that.data.approve_remark;
      wx.request({
        url: app.globalData.address + 'change/toApprove.do',
        method: 'POST',
        header: {
          "Authorization": wx.getStorageSync('tokenValue'),
          "Content-Type": "application/json"
