...
Page({
  ...
  approveRemark: function (e) { //描述
    this.setData({
      after_implement_remark: e.detail
    })
  },
  backTo() {
    wx.navigateBack({
  ...
  saveClick: function (event) {
    var that = this;
    that.data.form.changeImplementApprove.after_implement_remark = that.data.after_implement_remark;
    if(!that.data.after_implement_remark){
      wx.showToast({
        title: '请输入实施后评审意见',
        icon: 'error',
        duration: 2000
      ...
      wx.request({
        url: app.globalData.address + 'change/toImplementApprove.do',
        method: 'POST',
        header: {
          "Authorization": wx.getStorageSync('tokenValue'),
          "Content-Type": "application/json"
