...
Page({
  ...
  page: function (val) {
    var that = this;
    wx.request({
      url: app.globalData.address + 'change/toNeedList.do',
      method: 'POST',
      header: {
        "Authorization": wx.getStorageSync('tokenValue'),
        "Content-Type": "application/json"
  ...
  remarksAddClick(event) {
    var that = this;
    if (null == that.data.detailId || that.data.detailId == '') {
      wx.showToast({
        title: '请选择一条数据',
        icon: 'none',
        duration: 2000
  ...
  reopenAddClick(event) {
    ...
    wx.request({
      url: app.globalData.address + 'change/toSuspend.do',
      method: 'POST',
      header: {
        "Authorization": wx.getStorageSync('tokenValue'),
        "Content-Type": "application/json"
  ...
  showaction: function (event) {
    this.data.detailId = event.currentTarget.dataset.id.id;
    this.data.old_status = event.currentTarget.dataset.id.old_status;
    var status = event.currentTarget.dataset.id.status;
    this.data.actions = [];
    this.data.actions.push({
      name: '详情'
    })
    var authority = wx.getStorageSync('authority');
    if (authority.change_dealtWith_zhuandan && (status == 3 || status == 5 || status == 6)) {
      this.data.actions.push({
        name: '转单'
      })
    } else if (authority.change_dealtWith_huifu && status == 11) {
      this.data.actions.push({
        name: '恢复'
      })
    } else if (authority.change_dealtWith_beizhu) {
      this.data.actions.push({
        name: '备注'
      })
    } else if (authority.change_dealtWith_guaqi && (status == 3 || status == 5 || status == 6)) {
      this.data.actions.push({
        name: '挂起'
      })
    }
  ...
  onSelect(event) {
    ...
    if (event.detail.name == "转单") {
      wx.navigateTo({
        url: 'zhuandan/index?id=' + that.data.detailId,
      })
      ...
      Dialog.confirm({
          ...
          wx.request({
            ...
            data: {
              "userId": wx.getStorageSync('userInfo').id,
              "id": that.data.detailId,
              "old_status": that.data.old_status
            },
            success: function (res) {
              if (res.data.code == 0) {
                wx.showToast({
      ...
      wx.navigateTo({
        url: 'detail/index?id=' + that.data.detailId,
      })
    }
  },
  ...
  shClick: function (event) {
    wx.navigateTo({
      url: 'shenhe/index?id=' + event.currentTarget.dataset.id,
    })
  },
  zdfaClick: function (event) {
    wx.navigateTo({
      url: 'fangan/index?id=' + event.currentTarget.dataset.id,
    })
  },
  fashClick: function (event) {
    wx.navigateTo({
      url: 'fanganshenhe/index?id=' + event.currentTarget.dataset.id,
    })
  },
  shishiClick: function (event) {
    wx.navigateTo({
      url: 'shishi/index?id=' + event.currentTarget.dataset.id,
    })
  },
  shishipsClick: function (event) {
    wx.navigateTo({
      url: 'shiships/index?id=' + event.currentTarget.dataset.id,
    })
  },
  guandanClick: function (event) {
    var that = this;
    Dialog.confirm({
        title: '提示',
        message: '确定要关单吗？',
