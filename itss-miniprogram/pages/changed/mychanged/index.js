...
Page({
  ...
  page: function (val) {
    var that = this;
    wx.request({
      url: app.globalData.address + 'change/toList.do',
      method: 'POST',
      header: {
        "Authorization": wx.getStorageSync('tokenValue'),
        "Content-Type": "application/json"
      ...
      data: {
        "operate_user": wx.getStorageSync('userInfo').id,
  ...
  remarksAddClick(event) {
    ...
    if (null == that.data.shuoming || that.data.shuoming == '' || that.data.shuoming == undefined) {
      wx.showToast({
        title: '请输入备注说明',
        icon: 'none',
        duration: 2000
  ...
  showaction: function (event) {
    ...
    this.data.actions = [];
    if (authority.change_mechange_beizhu) {
      this.data.actions.push({
        name: '备注'
      });
    }
    if (authority.change_mechange_chexiao && (status == 1 || status == 2)) {
      this.data.actions.push({
        name: '撤销'
      });
    }
