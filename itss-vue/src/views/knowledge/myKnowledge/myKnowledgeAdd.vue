      ...
      <el-row :gutter="20">
        <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
          <el-card class="cardbox" shadow="never">
            ...
            <el-col :xs="24" :sm="24" :md="6" :lg="6" :xl="6">
              <el-form-item label="知识类别" prop="knowledge_category_name">
                <el-input
                  autocomplete="off"
                  placeholder="请选择"
                  v-model="form.knowledge_category_name"
                  style="width: 80%"
                  @click.native="openwinThree"
                ></el-input>
                <el-button
                  icon="el-icon-search"
                  style="
            ...
            <el-col :xs="24" :sm="24" :md="6" :lg="6" :xl="6">
              <el-form-item label="可被搜索" prop="is_full_search">
                <el-select
                  v-model="form.is_full_search"
                  placeholder="请选择"
                  style="width: 100%"
                >
            ...
            <el-col :xs="24" :sm="24" :md="6" :lg="6" :xl="6">
              <el-form-item
                label="工程师内部分享范围"
                prop="knowledge_range_id"
              >
                <el-select
                  v-model="form.knowledge_range_id"
                  placeholder="请选择"
                  style="width: 100%"
                >
...
export default {
  ...
  created() {
    ...
    if (typeof query != 'undefined' && query != null && query != '') {
      //标题回显
      this.form.title = query.bean.title
      //正文回显
      this.form.content = query.bean.description
    }
  ...
  methods: {
    ...
    saveok() {
      ...
      console.log(this.form.keyword.length)
      if(this.form.title.length > 255){
        this.$baseMessage("标题字数过多", 'warning')
        return
      }
      if(this.form.keyword.length > 255){
        this.$baseMessage("关键词字数过多", 'warning')
        return
      }
      this.$refs['form'].validate(async (valid) => {
        if (valid) {
          ...
          const { msg } = await addKnowledgeInfo(params)
          console.log(msg)
