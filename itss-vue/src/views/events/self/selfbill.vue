...
<!--          </el-col>-->
      ...
      <vab-query-form>
        <vab-query-form-left-panel :span="8">
          <el-button v-permission="event_self_kaidan" icon="el-icon-plus" type="primary" @click="handleAdd">开单</el-button>
          <el-button v-permission="event_self_zichanpandian" icon="el-icon-files" @click="handelCapital">资产盘点</el-button>
        </vab-query-form-left-panel>
        <vab-query-form-right-panel :span="16">
          <el-form-item>
            <el-date-picker v-model="queryForm.create_time_start" placeholder="开单时间(Start)" value-format="yyyy-MM-dd" type="date" clearable></el-date-picker>
          </el-form-item>
          <el-form-item>
            <el-date-picker v-model="queryForm.create_time_end" placeholder="开单时间(End)" value-format="yyyy-MM-dd" type="date" clearable></el-date-picker>
          </el-form-item>
          <el-form-item>
            <el-input v-model.trim="queryForm.title" placeholder="关键字" clearable @keyup.enter.native="queryData"/>
          </el-form-item>
          <el-form-item>
            <el-button v-permission="event_self_queryList" icon="el-icon-search" type="primary" @click="queryData">查询</el-button>
...
<!--        </template>-->
      ...
      <el-table-column show-overflow-tooltip prop="statusName" align="center" label="状态" width="100px;">
        <template slot-scope="scope">
      ...
      <el-table-column show-overflow-tooltip label="操作" width="200" align="center" fixed="right">
        <template #default="{ row }">
          <el-button v-permission="event_self_xiugai" plain @click="handleEdit(row)" type="primary" size="mini" v-if="row.status == 1">修改</el-button>
          ...
          <el-button v-permission="event_self_querenpingjia" plain @click="handlePj(row)" type="primary" size="mini" v-if="row.status == 5">确认评价</el-button>
          <el-dropdown trigger="click" @command="handleCommandmore" style="margin-left: 10px" v-if="row.status == 1 || row.status == 2 || row.status == 3">
            ...
            <el-dropdown-menu slot="dropdown">
              <!--<el-dropdown-item v-permission="event_self_querenpingjia" icon="el-icon-connection" :command="commandmoreValue('a',row)" v-if="row.status == 5">确认评价</el-dropdown-item>-->
              <el-dropdown-item v-permission="event_self_beizhu" icon="el-icon-reading" :command="commandmoreValue('c',row)">备注</el-dropdown-item>
              <el-dropdown-item v-permission="event_self_chexiao" icon="el-icon-refresh-right" :command="commandmoreValue('d',row)">撤销</el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
        </template>
      </el-table-column>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        event_self_kaidan: 'event_self_kaidan',
        event_self_zichanpandian: 'event_self_zichanpandian',
        event_self_queryList: 'event_self_queryList',
        event_self_xiugai: 'event_self_xiugai',
        event_self_chongkai: 'event_self_chongkai',
        event_self_querenpingjia: 'event_self_querenpingjia',
      ...
      handleCommandmore(val) {
        ...
        switch (command) {
          case 'a':
            ...
            break
          case 'b':
            this.$refs['reopen'].showEdit(val.row)
            break
          case 'c':
            this.$router.push({
