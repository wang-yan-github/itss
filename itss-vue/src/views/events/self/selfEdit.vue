<template>
  <div class="main-container pb80">
    <el-form  ref="form" :model="form" :rules="rules"  label-width="120px">
                      ...
                      <el-row :gutter="20">
                        <el-col :xs="24" :sm="24" :md="6" :lg="6" :xl="6">
                          <el-form-item label="服务类别" prop="parent_type_name" >
                            <el-input autocomplete="off" placeholder="请选择" style="width: 80%;" clearable  @click.native="openwinOne"
                        ...
                        <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
                         ...
                         <el-col :xs="24" :sm="24" :md="6" :lg="6" :xl="6">
                          <el-form-item label="关联的配置项" prop="eventAssetName">
                            <el-input autocomplete="off" placeholder="请选择" style="width: 80%;" @click.native="handleAddpz"
                                      v-model="form.eventAssetName"></el-input>
                            <el-button icon="el-icon-search"
                                      style="border-left: 0; background-color: #f5f7fa; height: 32px; width: 20%"
                                      @click="handleAddpz"></el-button>
                          </el-form-item>
                        </el-col>
                          <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
                          ...
                          <el-col :xs="24" :sm="24" :md="4" :lg="4" :xl="4">
                            <el-form-item label="请求类型:"   prop="request_type">
                              <el-select v-model="form.request_type" placeholder="请选择" >
                                <el-option
                                  v-for="item in eventRequestTypeList"
                                  :key="item.id"
      ...
      <assetsManagesDialog ref="assetsManagesDialog" @listData="listAssetsManages"  @fetch-data="{inputval}"></assetsManagesDialog>
  </div>
</template>
<script>
...
import {categoryList,saveWantToBill,updateWantToBill} from '@/api/eventAllManage'
export default {
   name: 'selfEdit',
   ...
   data() {
    return {
        ...
        rules: {
          parent_type_name: [{required: true, trigger: 'submit', message: '请选择'}],
          title: [{required: true, trigger: 'blur', message: '请输入'}],
          description: [{required: true, trigger: 'blur', message: '请输入'}],
          eventAssetName: [{required: true, trigger: 'submit', message: '请选择'}],
          request_type: [{required: true, trigger: 'blur', message: '请选择'}],
          urgency_degree_id: [{required: true, trigger: 'blur', message: '请输入'}],
          effect_degree: [{required: true, trigger: 'blur', message: '请输入'}]
      },
    }
   },
  computed: {
    changeData() {
      if (
        ...
        categoryList(this.queryCategroyForm).then((res) => {
          console.log("computed-changeData-categoryList:",res)
          if (res.code == 0) {
            if (res.data == 1) {
  ...
  mounted() {},
   ...
   methods: {
       async getData() {
        ...
        const {data} = await updateWantToBill(query.id)
        this.form = data;
        this.queryCategroyForm.first_id = data.priority_id;//考虑优先级
        this.queryCategroyForm.eventCategoryId  = data.event_category_id;//工单类别
        this.queryCategroyForm.request_type_id  = data.request_type;//考虑请求类型
        this.categoryListClick();
        //属性信息 附件
        if (this.form.files) {
            this.form.files.forEach((x) => {
            this.WorkFileList.push({name: x.file_name, url: x.id})
          })
        }
      ...
      getBillType(data) {
        var that = this;
        this.form.event_category_id=data.worker_order_id;//工单类别id
        this.form.parent_type_name = data.name;
      ...
      categoryListClick() {
        ...
        (this.form.request_type != null && this.form.request_type != undefined && this.form.request_type != '')){
          ...
          this.queryCategroyForm.eventCategoryId =this.form.event_category_id;//工单类别
          this.queryCategroyForm.request_type_id = this.form.request_type;//考虑请求类型
          console.log(this.queryCategroyForm)
          categoryList(this.queryCategroyForm).then((res) => {
            if (res.code == 0) {
              if(res.data == 1 ){
                this.ResponseSLA = "暂无";
