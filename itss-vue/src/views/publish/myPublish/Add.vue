            ...
            <div class="box_card">
              ...
              <div class="box_content">
                <el-row :gutter="20">
                  ...
                  <el-col :xs="24" :sm="24" :md="6" :lg="6" :xl="6">
                    <el-form-item label="发布人:">{{ form.publisherName }}</el-form-item>
                  </el-col>
                  ...
                  <el-col :xs="24" :sm="24" :md="6" :lg="6" :xl="6">
                    <el-form-item label="测试人:" prop="tester_id">
...
<script>
  import {getPublishBasicInfo, getAllCategoryList, getPublishCategory, publishBasicInfoToAdd} from '@/api/publish'
  import {getUserList, toList} from '@/api/change'
  import vabQuill from '@/plugins/vabQuill'

  // 引用变更弹窗页面
  import changeDialog from '@/components/changeDialog'
  import {toQuestionAdd} from "@/api/question";
  ...
  export default {
    ...
    data() {
      return {
        ...
        rules: {
          ...
          tester_id: [{required: true, trigger: 'blur', message: '请选择测试人'}],
        },
      }
    },
    created() {
      //变更生成发布，接收问题参数对象
      const query = this.$route.query.changeInfo;
      console.log("变更生成发布")
      console.log(query)
      if (typeof query != 'undefined' && query != null && query != '') {
        const changeInfo = query.changeInfo;
        changeInfo.classify_name = query.classify_name;
        changeInfo.type_name = query.type_name;
        changeInfo.status_name = query.status_name;
        this.tableData.push(query.changeInfo);
        this.changeInfoSize=1;
      }
    ...
    methods: {
      ...
      handleAdd() {
        this.$refs['changeDialog'].showWin(this.tableData)
      },
      handlePreview(file) {
        console.log(file)
      },
      ...
      saveok() {
        if (this.form.title.length > 256){
          this.$baseMessage("标题超出限制", 'warning')
          return
        }
        this.$refs['form'].validate(async (valid) => {
          if (valid) {
            ...
            this.disabled = true;
            publishBasicInfoToAdd(params).then((res) => {
              this.$baseMessage("成功", 'success')

              if(this.changeInfoSize==1){
                this.$store.dispatch('tabsBar/delRoute', this.$route)
                this.$router.push('/biangeng/changeDeal')
              }else{
      ...
      async fetchPublishCategoryLData() {
        ...
        const {data} = await getAllCategoryList({is_use: '1'})
        console.log(data)
      ...
      async getPublishCategory() {
        ...
        var x = "";
        for(var i = 0 ; i < data.publishApproveInfos.length ; i++){
          if(i == data.publishApproveInfos.length -1){
            x += data.publishApproveInfos[i].name;
          }else {
            x += data.publishApproveInfos[i].name;
            x += ","
          }
