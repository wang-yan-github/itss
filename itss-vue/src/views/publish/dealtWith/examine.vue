                  ...
                  <div class="box_content">
                    ...
                    <el-row :gutter="20">
                      ...
                      <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
                        <el-form-item label="评审意见:">
                          <quill-editor
                            v-model="publishApprovalInfos.approval_opinions"
                            :min-height="200"
                            :options="options"
                          ></quill-editor>
                          <p class="ts">
...
<!--                  ></el-table-column>-->
      ...
      <div class="bottom">
        <el-button type="primary" size="medium" @click="approvePublish('1')">
          通过
        </el-button>
        <el-button type="warning" size="medium" @click="approvePublish('2')">
          拒绝
        </el-button>
      </div>
    </el-form>
...
import {getPublishBasic,approvePublish,getApprovalByPublishId,getLogInfo} from "@/api/publish";
...
export default {
  ...
  data() {
    return {
      ...
      form: {
        ...
        changeVoList:[],
        approval_opinions: '',
        publishApprovalInfo:{approval_opinions:''},
      },
      // approval_opinions:'',
      publishApprovalInfos:{approval_opinions:''},
      approvalInfos:[],
      rules: {
        username: [
  ...
  methods: {
    ...
    async getPublishBasic() {
      ...
      this.publishApprovalInfos.approval_opinions=(this.form.publishApprovalInfo==null?"":this.form.publishApprovalInfo.approval_opinions)
      setTimeout(() => {
        this.listLoading = false
      }, 300)
    },
    async approvePublish(isPass) {
      this.form.isPass=isPass
      console.log(this.publishApprovalInfos.approval_opinions);
      this.form.approval_opinions=this.publishApprovalInfos.approval_opinions;
      const params = new FormData();
      params.append('body', encodeURIComponent(JSON.stringify(this.form)));
      const {data} = await approvePublish(params)
