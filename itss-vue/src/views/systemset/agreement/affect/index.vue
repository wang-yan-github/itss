    ...
    <vab-query-form>
      ...
      <vab-query-form-right-panel :span="16">
        <el-form :inline="true" :model="queryForm" @submit.native.prevent>
          ...
          <el-form-item>
            <el-input
              v-model.trim="queryForm.impact_name"
              placeholder="名称"
              clearable
    ...
    >
      ...
      >
        <template #default="scope">
          <div
            v-if="scope.row.default_value == '0'"
          >{{scope.row.impact_name}}<span style="color:red">[默认]</span></div
          >
          <div
            v-else-if="scope.row.default_value == '1'"
          >{{scope.row.impact_name}}
          </div
          >
        </template>
      </el-table-column>
...
<script>
  ...
  import {affectGetList, affectToEdit} from '@/api/affect'
  ...
  export default {
    ...
    methods: {
      ...
      handleDelete() {
        ...
        if (this.selectRows != '' && this.selectRows != null) {
          ...
          this.$baseConfirm('你确定要删除选中项吗', null, async () => {
            const {msg} = await affectToEdit({id: ids, is_del: '1'})
      ...
      async fetchData() {
        ...
        const {data} = await affectGetList(this.queryForm)
        console.log(data);
        this.list = data.list
