    ...
    <vab-query-form>
      ...
      <vab-query-form-right-panel :span="16">
        <el-form :inline="true" :model="queryForm" @submit.native.prevent>
          ...
          <el-form-item>
            <el-input
              v-model.trim="queryForm.urgent_name"
              placeholder="名称"
              clearable
              @keyup.enter.native="queryData"
            />
    ...
    >
      ...
      >
        <template #default="scope">
          ...
          >{{scope.row.urgent_name}}<span style="color:red">[默认]</span></div
          >
          <div
...
<script>
  import {urgencyGetList, urgencyToEdit} from '@/api/affect'
  ...
  export default {
    ...
    methods: {
      ...
      handleDelete() {
        if (this.selectRows != '' && this.selectRows != null) {
          ...
          this.$baseConfirm('你确定要删除选中项吗', null, async () => {
            const {msg} = await urgencyToEdit({id: ids, is_del: '1'})
      ...
      async fetchData() {
        ...
        const {data} = await urgencyGetList(this.queryForm)
        this.list = data.list
