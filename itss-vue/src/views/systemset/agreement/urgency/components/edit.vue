<template>
  ...
  >
    <el-form ref="form" :model="form" :rules="rules" label-width="120px">
      <el-form-item label="名称" prop="urgent_name">
        <el-input v-model.trim="form.urgent_name" autocomplete="off"></el-input>
      </el-form-item>
...
<script>
  import {urgencyToAdd, urgencyToEdit} from "@/api/affect";
      ...
      save() {
        ...
        this.$refs['form'].validate(async (valid) => {
          if (valid) {
            try{
              if (this.idx === '1') {
                const data = await urgencyToAdd(this.form)
                if (data.code == 0) {
                ...
                const data = await urgencyToEdit(this.form)
                if (data.code == 0) {
                  this.$baseMessage(data.msg, 'success');
                  this.$emit('fetch-data');
                  this.close();
