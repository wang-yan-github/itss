...
<script>
  import {slaQuestionGetList, slaQuestionToEdit} from '@/api/affect'
  ...
  export default {
    ...
    methods: {
      ...
      handleDelete() {
        if (this.selectRows != '' && this.selectRows != null) {
          ...
          this.$baseConfirm('你确定要删除选中项吗', null, async () => {
            const {msg} = await slaQuestionToEdit({id: ids, is_del: '1'})
      ...
      async fetchData() {
        ...
        const {data} = await slaQuestionGetList(this.queryForm)
        this.list = data.list
