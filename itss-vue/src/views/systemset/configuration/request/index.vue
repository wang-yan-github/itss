    ...
    <vab-query-form>
      ...
      <vab-query-form-right-panel :span="16">
        <el-form :inline="true" :model="queryForm" @submit.native.prevent>
          ...
          <el-form-item>
            <el-input
              v-model.trim="queryForm.request_type_name"
              placeholder="名称"
              clearable
    ...
    >
      ...
      >
        <template #default="scope">
          <div v-if="scope.row.is_default == '1'">{{scope.row.request_type_name}}<span style="color:red">[默认]</span>
          </div>
          <div v-else>{{scope.row.request_type_name}}</div>
        </template>
      </el-table-column>
      <el-table-column
        show-overflow-tooltip
...
<script>
  import {getList, del} from '@/api/eventRequestType'
