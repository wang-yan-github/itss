...
<script>
  ...
  import {getAssetsConfigStatusList, delAssetsConfigStatus, getById} from "@/api/assetsConfigStatus";
  ...
  export default {
    ...
    methods: {
      ...
      handleAdd() {
        const assets_id = this.queryForm.assets_id
        this.$router.push({path: this.$refs['edit'].showEdit(), query: {assets_id: assets_id}})
      },
      async handleView() {
        if (this.selectRows.id) {
          ...
          // this.$router.push({
          //   path:
          //   query: {name: data.name, is_enable: data.is_enable, sort: data.sort, code: data.code}
          // })
        } else {
      ...
      handleEdit() {
        if (this.selectRows.id) {
          ...
          this.$router.push({
            ...
            query: {id: this.selectRows.id, assets_id: assets_id}
          })
      ...
      handleDelete() {
        if (this.selectRows != '' && this.selectRows != null) {
          ...
          this.$baseConfirm('你确定要删除选中项吗', null, async () => {
            const {msg} = await delAssetsConfigStatus({ids})
      ...
      async fetchData() {
        ...
        const assets_id = this.$route.query.id
        this.queryForm.assets_id = assets_id
        const {data, totalCount} = await getAssetsConfigStatusList(this.queryForm)
        this.list = data.list
