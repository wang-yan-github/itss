<template>
  <div class="index-container pb80">
            ...
            <el-row :gutter="20">
              <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
                <el-card class="cardbox" shadow="hover">
                  ...
                  <el-col :xs="24" :sm="24" :md="6" :lg="6" :xl="6">
                    <el-form-item label="配置子类" prop="assets_children_id">
                      <el-select v-model="form.childAssetName" class="w" clearable>
                  ...
                  <el-col :xs="24" :sm="24" :md="6" :lg="6" :xl="6">
                    <el-form-item label="负责人">
                      <el-input
                        ...
                        placeholder="负责人"
                        clearable
                        @click.native="handleSolveUser('fzr')"
                      >
                  ...
                  <el-col :xs="24" :sm="24" :md="6" :lg="6" :xl="6">
                    <el-form-item label="领用人">
                      <el-input autocomplete="off" placeholder="领用人" v-model.trim="form.receiver_name"
                                @click.native="handleSolveUser('lyr')"></el-input>
              ...
              <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24" v-show="customFields.length > 0">
                <el-card class="cardbox" shadow="hover">
                  ...
                  <el-col :xs="24" :sm="24" :md="6" :lg="6" :xl="6" v-for="(item,index) in this.customFields"
                          :key="index">
                    ...
                    <el-form-item v-if="item.type == 2">
                      <label slot="label">{{item.name}}</label>
                      <el-input v-model="dyFieldItems[item.id]" type="number" placeholder="请输入内容"
                    ...
                    <el-form-item v-if="item.type == 5">
                      ...
                      <el-radio-group v-model="dyFieldItems[item.id]">
                        <el-radio v-for="(fieldItem,index) in item.fieldVal" :key="fieldItem.fId" :label="fieldItem.fId"
                                  @change="change1">{{fieldItem.fVal}}
                        </el-radio>
                      </el-radio-group>
                    </el-form-item>
    ...
    <div class="bottom">
      <el-button type="primary" size="medium" :loading="loading" @click="saveok">{{ loadingbuttext }}</el-button>
...
<script>
  ...
  export default {
    ...
    created() {
      ...
      this.form.assets_type_id = this.$route.query.assets_type_id //配置项id
      this.typeForm.id = this.$route.query.assets_type_id
      this.getAssetsChildren()//获取配置项对应的所有子类配置
      // this.getCustomFiled()
      ...
      //this.form.assets_type_id  = query.id
    ...
    methods: {
      ...
      async getData(val) {
        ...
        this.form = data;
        this.form.child_asset = data.child_asset;
        this.form.childAssetName = data.childAssetName;
        ...
        //动态字段 开始
        var jsonStr = data.extend;
        // debugger
        if (jsonStr != undefined && jsonStr != null && jsonStr != "") {
          var json = JSON.parse(jsonStr);
          for (var i in json) {
            this.dyFieldItems[i] = json[i];
            if (i.startsWith("cBox__")) {
              var index = Number(i.substr(6));
              this.dyFieldItemsCheck[index] = json[i]
            }
      ...
      // },
      handleAdd2() {
        this.$refs['dialogthree'].showWin(this.contractContactList)
      },
      handleAdd3() {
        this.$refs['dialogfour'].showWin(this.supplierContractList)
      },
      handleAdd4() {
        this.$refs['dialogfive'].showWin(this.assetsTypeList)
      },
      remoteMethod(query) {
        if (query !== '') {
          this.loading = true
      ...
      saveok() {
        ...
        that.$refs['form'].validate((v) => {
          if (v) {
            that.loading = true;
            that.loadingbuttext = '修改中...';
            const params = new FormData()

            const uploadedFiles = []
            const uploadedpictures = []
            for (let i = 0; i < that.fileList.length; i++) {
              const f = that.fileList[i]
              if (f.status == 'ready') {
            ...
            let newarr = that.contractContactList.map((item) => {
              return item.id
            })
            let newarr1 = that.supplierContractList.map((item) => {
              return item.id
            })
            let newarr2 = that.assetsTypeList.map((item) => {
              return item.id
            })
            if (newarr2.length > 0) {
              that.form.assetsIds = newarr2.join(',')
            }
            if (newarr1.length > 0) {
              that.form.supplierIds = newarr1.join(',')
            }
            if (newarr.length > 0) {
              that.form.contractIds = newarr.join(',')
            }
            this.form.uploadedFiles = uploadedFiles.join(',')
            this.form.uploadedpictures = uploadedpictures.join(',')
            params.append('body', JSON.stringify(that.form))
            console.log(params)
