...
<template>
  <el-dialog :title="title" :visible.sync="dialogFormVisible" append-to-body width="900px" @close="close" append-to-body>
    <vab-query-form>
      <vab-query-form-left-panel :span="24">
        <el-form :inline="true" :model="queryForm" @submit.native.prevent>
          <el-form-item>
            <el-input
              v-model.trim="queryForm.sla_question_name"
              placeholder="名称"
              clearable
    ...
    <el-table v-loading="listLoading" :data="tableData" border highlight-current-row
      ...
      <el-table-column show-overflow-tooltip prop="sla_question_name" align="center" sortable label="名称">
      </el-table-column>
      <el-table-column
      ...
      <el-table-column show-overflow-tooltip prop="is_first" align="center" label="考虑优先级" sortable>
        <template #default="scope">
          ...
          <el-tag
            size="small"
            type="warning"
            v-else-if="scope.row.is_first == '1'"
          >是
          </el-tag
          >
        </template>
...
<script>
  import {questionSlaGetList} from '@/api/affect'
  ...
  export default {
    ...
    methods: {
      ...
      async fetchData() {
        ...
        const {data} = await questionSlaGetList(this.queryForm);
